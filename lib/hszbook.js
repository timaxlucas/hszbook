...
async function registerForCourse(link, kid, data, dry = false, { retryDelay = _RETRY_DELAY_MS, debug = _DEBUG_MODE, retryNum = _RETRY_NUM } = {}) {
  ...
  const soldOut = await page.evaluate(() => document.querySelector('body').innerText.includes('Dieses Angebot ist leider schon ausgebucht!'));

  if (soldOut) {
    logger.error(`booking failed: sold out`, { source: 'scraper' });
    await browser.close();
    return {success: false, message: `already sold out`};
  }
  ...
  if (!success) {
    ...
    const alreadySigned = await page.evaluate(() => document.querySelector('body').innerText.includes('Sie sind für dieses Angebot bereits'));
    let message = '';

    if (notAllowedError)
      message = 'You are not allowed to register for this course online';
    else if (alreadySigned)
      message = 'You are already registered to this course';
    else
      message = `Unknown Error: please see ${screen_path}`;
