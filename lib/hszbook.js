...
function startBrowser(headless = _DEBUG_MODE) {
  ...
  return puppeteer.launch({ devtools: headless, args:['--no-sandbox'] });
}
...
async function registerForCourse(link, kid, data, { retryDelay = _RETRY_DELAY_MS, debug = _DEBUG_MODE, retryNum = _RETRY_NUM }) {
  ...
  let pickTimeBool = await page.evaluate(() => {
    return document.querySelector("body").innerText.includes("Bitte wählen Sie einen Termin aus:");
  });
  ...
  let confirmEmail = await page.evaluate(() => {
    return document.querySelector("body").innerText.includes("Bitte geben Sie zur Sicherheit Ihre Emailadresse noch einmal ei");
  });
  ...
  await wait(3000);
  let screen_path = `result_${moment().format('HH-mm_DD.MM.YYYY')}.png`;
  await page.screenshot({ path: screen_path });

  let success = await page.evaluate(() => {
    return document.querySelector("body").innerText.includes("Bestätigung");
  });
  if (!success) {
    let notAllowedError = await page.evaluate(() => {
      return document.querySelector("body").innerText.includes("Für Sie ist eine Anmeldung über das");
    });
    //logger.error((notAllowedError ? "NotAllowedError" : `Unknown Error: please see ${screen_path}`), { source: 'scraper' });
...
async function getData(url) {
  ...
  let data = await page.evaluate(() => {
    ...
    tables.forEach(table => {
      ...
      t.forEach(entry => {
        let but = entry.querySelector(".bs_sbuch input");
        let ready = false;
        let x;
        if (but == null) {
  ...
  data = data.map((d) => {
    if (d.canBook) {
      ...
      let month = parseInt(d.state.split(".")[1])-1,
          day = parseInt(d.state.split(".")[0]),
          hour = parseInt(d.state.split(',')[1].split(':')[0]),
          minute = parseInt(d.state.split(',')[1].split(':')[1]);
