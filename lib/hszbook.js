...
function startBrowser(headless = _DEBUG_MODE) {
  if (isDocker()) {
    logger.debug('launching browser in docker mode', { source: 'scraper' });
  ...
  logger.debug('launching browser in default mode with headless: ' + headless, { source: 'scraper' });
...
async function registerForCourse(link, kid, data, dry = false, { retryDelay = _RETRY_DELAY_MS, debug = _DEBUG_MODE, retryNum = _RETRY_NUM } = {}) {
  ...
  const dataValid = await page.evaluate(() => document.querySelector('body').innerText.includes('Bitte überprüfen Sie noch einmal Ihre Angaben'));

  // dry run, so exit
  if (dry) {
    await browser.close();
    return { success: dataValid, message: 'DRY'};
  }
